  ...
  methods: {
    async reload() {
      try {
        ...
        if (this.getCompanyList.nextmApiResult.errorCode == 200) {
          this.companyList = this.getCompanyList.nextmApiResult.menuMemberList;
        } else {
    ...
    async companyDetail(id) {
      try {
        ...
        if (this.getCompanyDetail.nextmApiResult.errorCode == 200) {
          this.companyId = id;
          //총 갯수
          const count = Object.keys(this.getCompanyDetail.nextmApiResult.menuMemberList).length;
          //새로운 배열
          const list = [];
          for (let i = 0; i < count; i++) {
            await list.push(this.getCompanyDetail.nextmApiResult.menuMemberList[i]);
          }
          ...
          this.alim(this.getCompanyDetail.nextmApiResult.errorMessage, this.errorColor);
        }
      } catch (error) {
        this.alim(error, this.errorColor);
      } finally {
      ...
      if (data.del === "Y") {
        if (data.type === "list") {
          try {
            ...
            if (this.powerDeleteResult.nextmApiResult.errorCode == 200) {
              this.reload();
              this.menuDetalList = [];
              this.alim("삭제 되었습니다.", this.successColor);
            } else {
              this.alim(this.powerDeleteResult.nextmApiResult.errorMessage, this.errorColor);
            }
          } catch (error) {
            this.alim(error, this.errorColor);
          } finally {
